#requires python3
#creates an output called hcsSS_codeContributors.txt
import subprocess, os

if os.path.exists("Projectnamesforanalysis.txt"):
    
    #remove codeContributors file to start a new one
    if os.path.exists("hcsSS_codeContributors.txt"):
        os.remove("hcsSS_codeContributors.txt")
    ...
    with open("Projectnamesforanalysis.txt") as projectFolders:
        for projectFolder in projectFolders:
            ...
            with open("hcsSS_codeContributors.txt", "a") as external_file:
                add_text = "project|tag|numberOfCodeContributors"
            ...
            #for each tag, identify code contributors 
            #We consider a developer as code contributor
            #if she/he has at least one commit in a tag 
